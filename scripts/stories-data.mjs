 ...
 * Anime entries include nameJa for English + Japanese guess acceptance.
 */
...
function anime(id, name, nameJa, tier, hints, facts, aliases = []) {
  return story({
    id,
    name,
    nameJa,
...
function tv(id, name, tier, hints, facts, nameJa, aliases = []) {
  return story({
    id,
    name,
    nameJa,
...
export const STORIES = [
  // ─── Anime Tier A ─────────────────────────────────────────────
  anime("one-piece", "One Piece", "ワンピース", "a", ["Japanese anime", "About pirates", "Very long-running series"], ["Monkey D. Luffy searches for the ultimate treasure left by Gol D. Roger."]),
  anime("naruto", "Naruto", "ナルト", "a", ["Japanese anime", "Ninja world", "Orange jumpsuit hero"], ["A young ninja seeks recognition and dreams of becoming Hokage."]),
  anime("dragon-ball", "Dragon Ball", "ドラゴンボール", "a", ["Japanese anime", "Martial arts and aliens", "Collects magical orbs"], ["Goku and friends defend Earth while searching for Dragon Balls."], ["Dragon Ball Z", "ドラゴンボールZ"]),
  anime("pokemon", "Pokémon", "ポケットモンスター", "a", ["Japanese anime", "Creatures in balls", "Catch and battle them"], ["Trainers journey to become Pokémon Masters."], ["Pokemon", "ポケモン"]),
  anime("bleach", "Bleach", "ブリーチ", "a", ["Japanese anime", "Soul reapers", "Orange-haired teen fights spirits"], ["Ichigo Kurosaki gains powers to protect humans from hollows."]),
  anime("hunter-x-hunter", "Hunter x Hunter", "ハンター×ハンター", "a", ["Japanese anime", "Hunters and exams", "Young boy searches for his father"], ["Gon Freecss takes the Hunter Exam to find his father Ging."]),
  anime("jojo", "JoJo's Bizarre Adventure", "ジョジョの奇妙な冒険", "a", ["Japanese anime", "Generations of Joestars", "Stands and dramatic poses"], ["The Joestar family battles supernatural enemies across generations."]),
  anime("doraemon", "Doraemon", "ドラえもん", "a", ["Japanese anime", "Robot cat from the future", "Gadget pocket"], ["A robotic cat helps a boy named Nobita with future gadgets."]),
  anime("demon-slayer", "Demon Slayer", "鬼滅の刃", "a", ["Japanese anime", "Demons and swordsmen", "Taisho-era Japan"], ["Tanjiro Kamado fights demons after his family is slaughtered."], ["Kimetsu no Yaiba"]),
  anime("attack-on-titan", "Attack on Titan", "進撃の巨人", "a", ["Japanese anime", "Giant humanoid walls", "Military against titans"], ["Humanity lives behind walls fearing man-eating Titans."], ["Shingeki no Kyojin"]),
  anime("my-hero-academia", "My Hero Academia", "僕のヒーローアカデミア", "a", ["Japanese anime", "Superhero school", "Quirks are superpowers"], ["Izuku Midoriya trains to become the greatest hero."], ["Boku no Hero Academia"]),
  anime("jujutsu-kaisen", "Jujutsu Kaisen", "呪術廻戦", "a", ["Japanese anime", "Cursed spirits", "Modern-day sorcerers"], ["Yuji Itadori joins Jujutsu High to fight curses."]),
  anime("death-note", "Death Note", "デスノート", "a", ["Japanese anime", "Supernatural notebook", "Cat-and-mouse thriller"], ["A student uses a notebook that can kill anyone whose name is written in it."]),
  anime("chainsaw-man", "Chainsaw Man", "チェンソーマン", "a", ["Japanese anime", "Devils and hunters", "Chainsaw powers"], ["Denji merges with his dog Pochita to hunt devils for a normal life."]),
  anime("spy-x-family", "Spy x Family", "スパイファミリー", "a", ["Japanese anime", "Spy family comedy", "Cold War–like setting"], ["A spy, assassin, and telepath form a fake family for their missions."]),
  anime("sailor-moon", "Sailor Moon", "美少女戦士セーラームーン", "a", ["Japanese anime", "Magical girl team", "Moon-themed heroes"], ["Usagi Tsukino becomes Sailor Moon to fight evil."]),
  anime("evangelion", "Neon Genesis Evangelion", "新世紀エヴァンゲリオン", "a", ["Japanese anime", "Mecha and angels", "Psychological themes"], ["Teenagers pilot Evangelions to protect Earth from Angels."], ["Evangelion", "エヴァンゲリオン"]),
  anime("cowboy-bebop", "Cowboy Bebop", "カウボーイビバップ", "a", ["Japanese anime", "Space bounty hunters", "Jazz-inspired tone"], ["Spike Spiegel and crew chase bounties aboard the Bebop."]),
  anime("fma-brotherhood", "Fullmetal Alchemist: Brotherhood", "鋼の錬金術師", "a", ["Japanese anime", "Alchemy rules", "Two brothers seek a Philosopher's Stone"], ["Edward and Alphonse Elric try to restore their bodies after a failed ritual."], ["Fullmetal Alchemist", "鋼の錬金術師 FULLMETAL ALCHEMIST"]),
  anime("sword-art-online", "Sword Art Online", "ソードアート・オンライン", "a", ["Japanese anime", "Trapped in VR game", "Swords and levels"], ["Players are trapped in a VRMMORPG where death is real."]),
  anime("tokyo-ghoul", "Tokyo Ghoul", "東京喰種", "a", ["Japanese anime", "Ghouls eat humans", "Half-ghoul protagonist"], ["Ken Kaneki becomes part ghoul and is caught between two worlds."]),
  anime("spirited-away", "Spirited Away", "千と千尋の神隠し", "a", ["Japanese anime film", "Studio Ghibli", "Spirit bathhouse"], ["Chihiro works in a spirit world to save her parents."]),
  anime("totoro", "My Neighbor Totoro", "となりのトトロ", "a", ["Japanese anime film", "Studio Ghibli", "Forest spirits"], ["Two sisters befriend forest spirits including Totoro."]),
  anime("howls-moving-castle", "Howl's Moving Castle", "ハウルの動く城", "a", ["Japanese anime film", "Studio Ghibli", "Wizard and moving castle"], ["Sophie is cursed by a witch and finds refuge in Howl's castle."]),
  anime("princess-mononoke", "Princess Mononoke", "もののけ姫", "a", ["Japanese anime film", "Studio Ghibli", "Nature vs industry"], ["Ashitaka mediates conflict between forest gods and iron town."]),
  anime("your-name", "Your Name", "君の名は。", "a", ["Japanese anime film", "Body swap", "Comet disaster"], ["Two teenagers mysteriously swap bodies and try to find each other."], ["Kimi no Na wa"]),
  anime("akira", "Akira", "アキラ", "a", ["Japanese anime film", "Cyberpunk Neo-Tokyo", "Psychic powers"], ["A biker gang member gains devastating psychic powers in future Tokyo."]),
  anime("ghost-in-the-shell", "Ghost in the Shell", "攻殻機動隊", "a", ["Japanese anime", "Cyborg police", "Philosophical sci-fi"], ["Major Motoko Kusanagi hunts cyber-criminals in a networked future."]),
  anime("slam-dunk", "Slam Dunk", "スラムダンク", "a", ["Japanese anime", "Basketball", "Delinquent joins team"], ["Hanamichi Sakuragi joins his school basketball team and grows as a player."]),
  anime("detective-conan", "Detective Conan", "名探偵コナン", "a", ["Japanese anime", "Child detective", "Shrunken teenage sleuth"], ["Shinichi Kudo is transformed into a child and solves cases as Conan Edogawa."], ["Case Closed"]),
  anime("digimon-adventure", "Digimon Adventure", "デジモンアドベンチャー", "a", ["Japanese anime", "Digital monsters", "Kids in another world"], ["Children partner with Digimon to save both worlds."]),
  anime("yu-gi-oh", "Yu-Gi-Oh!", "遊戯王", "a", ["Japanese anime", "Card dueling", "Ancient Egyptian spirit"], ["Yugi Mutou solves puzzles and duels with the Millennium Puzzle's power."]),
  anime("captain-tsubasa", "Captain Tsubasa", "キャプテン翼", "a", ["Japanese anime", "Football/soccer", "Prodigy striker"], ["Tsubasa Ozora pursues his dream of winning the World Cup."]),
  anime("gintama", "Gintama", "銀魂", "a", ["Japanese anime", "Comedy samurai sci-fi", "Alien occupation of Edo"], ["Gintoki Sakata takes odd jobs in an alternate Edo period Japan."]),
  anime("code-geass", "Code Geass", "コードギアス", "a", ["Japanese anime", "Mecha and strategy", "Power to command obedience"], ["Lelouch vi Britannia leads a rebellion with the Geass power."], ["Code Geass Lelouch of the Rebellion"]),
  anime("one-punch-man", "One Punch Man", "ワンパンマン", "a", ["Japanese anime", "Superhero parody", "Hero defeats foes in one punch"], ["Saitama seeks a worthy opponent after becoming too strong from training."]),
  anime("mob-psycho", "Mob Psycho 100", "モブサイコ100", "a", ["Japanese anime", "Psychic powers", "Middle school esper"], ["Shigeo Kageyama suppresses his emotions while working for a fake psychic."]),
  anime("haikyu", "Haikyu!!", "ハイキュー!!", "a", ["Japanese anime", "Volleyball", "Short player spikes"], ["Shoyo Hinata and Tobio Kageyama push Karasuno toward nationals."]),
  anime("dragon-ball-super", "Dragon Ball Super", "ドラゴンボール超", "a", ["Japanese anime", "Sequel to Dragon Ball Z", "Gods of destruction"], ["Goku faces universes-level threats after the Buu saga."], ["DBS"]),
  anime("nausicaa", "Nausicaä of the Valley of the Wind", "風の谷のナウシカ", "a", ["Japanese anime film", "Studio Ghibli", "Post-apocalyptic ecology"], ["Nausicaä tries to prevent war between humans and the toxic jungle."]),
  anime("kiki-delivery-service", "Kiki's Delivery Service", "魔女の宅急便", "a", ["Japanese anime film", "Studio Ghibli", "Young witch delivery service"], ["Kiki trains as a witch and runs a delivery service by broomstick."]),
  anime("castle-in-the-sky", "Castle in the Sky", "天空の城ラピュタ", "a", ["Japanese anime film", "Studio Ghibli", "Floating castle Laputa"], ["Sheeta and Pazu seek a legendary castle in the sky."]),

  // ─── Anime Tier B ─────────────────────────────────────────────
  anime("kurokos-basketball", "Kuroko's Basketball", "黒子のバスケ", "b", ["Japanese anime", "Basketball", "Generation of Miracles"], ["Tetsuya Kuroko supports a team aiming to defeat his former middle school stars."]),
  anime("steins-gate", "Steins;Gate", "シュタインズ・ゲート", "b", ["Japanese anime", "Time travel", "Microwave experiments"], ["Okabe Rintaro discovers their lab can send messages to the past."]),
  anime("re-zero", "Re:Zero", "Re:ゼロから始める異世界生活", "b", ["Japanese anime", "Isekai", "Return by death"], ["Subaru Natsuki is summoned to another world and resets on death."]),
  anime("black-clover", "Black Clover", "ブラッククローバー", "b", ["Japanese anime", "Magic knights", "No-magic protagonist"], ["Asta aims to become Wizard King despite having no magic."]),
  anime("fairy-tail", "Fairy Tail", "フェアリーテイル", "b", ["Japanese anime", "Guild of mages", "Friendship power"], ["Natsu Dragneel and the Fairy Tail guild take magic requests."]),
  anime("vinland-saga", "Vinland Saga", "ヴィンランド・サガ", "b", ["Japanese anime", "Vikings", "Historical revenge"], ["Thorfinn seeks revenge against the man who killed his father."]),
  anime("blue-lock", "Blue Lock", "ブルーロック", "b", ["Japanese anime", "Football", "Ego-driven training program"], ["Japan's striker project isolates forwards to create a world-class egoist."]),
  anime("frieren", "Frieren: Beyond Journey's End", "葬送のフリーレン", "b", ["Japanese anime", "Elf mage", "After the hero party wins"], ["Frieren revisits places from her journey after the Demon King is defeated."]),
  anime("solo-leveling", "Solo Leveling", "俺だけレベルアップな件", "b", ["Japanese anime", "Hunters and dungeons", "Weakest becomes strongest"], ["Sung Jinwoo is the only hunter who can level up infinitely."]),
  anime("ponyo", "Ponyo", "崖の上のポニョ", "b", ["Japanese anime film", "Studio Ghibli", "Fish-girl and boy"], ["A goldfish princess wants to become human after meeting Sosuke."]),
  anime("initial-d", "Initial D", "頭文字D", "b", ["Japanese anime", "Street racing", "Mountain passes"], ["Takumi Fujiwara delivers tofu and becomes a legendary downhill racer."]),
  anime("madoka-magica", "Puella Magi Madoka Magica", "魔法少女まどか☆マギカ", "b", ["Japanese anime", "Magical girls", "Dark twist on genre"], ["Madoka Kaname is offered a contract to become a magical girl."]),
  anime("monogatari", "Monogatari Series", "物語シリーズ", "b", ["Japanese anime", "Supernatural oddities", "Wordplay-heavy dialogue"], ["Koyomi Araragi helps classmates afflicted by oddities and apparitions."]),
  anime("psycho-pass", "Psycho-Pass", "サイコパス", "b", ["Japanese anime", "Crime coefficient", "Dystopian Japan"], ["Inspectors hunt criminals judged by the Sibyl System's psycho-pass."]),
  anime("gundam", "Mobile Suit Gundam", "機動戦士ガンダム", "b", ["Japanese anime", "Mecha war", "Real robot genre pioneer"], ["Amuro Ray pilots the Gundam in the One Year War between Earth and colonies."]),
  anime("bocchi-the-rock", "Bocchi the Rock!", "ぼっち・ざ・ろっく！", "b", ["Japanese anime", "Rock band", "Social anxiety"], ["Hitori Gotoh joins a band and struggles to overcome her shyness."]),
  anime("oshi-no-ko", "Oshi no Ko", "【推しの子】", "b", ["Japanese anime", "Idol industry", "Reincarnation mystery"], ["A doctor reincarnates as the son of his favorite idol and uncovers dark secrets."]),
  anime("hells-paradise", "Hell's Paradise", "地獄楽", "b", ["Japanese anime", "Ninja on death island", "Immortality quest"], ["Gabimaru the Hollow seeks a pardon by finding the elixir of life on an island."]),
  anime("dr-stone", "Dr. Stone", "Dr.STONE", "b", ["Japanese anime", "Science rebuilds world", "Everyone petrified"], ["Senku Ishigami revives civilization using science after a global petrification."]),
  anime("fire-force", "Fire Force", "炎炎ノ消防隊", "b", ["Japanese anime", "Firefighters vs infernals", "Spontaneous human combustion"], ["Shinra Kusakabe joins Company 8 to fight infernals and uncover a conspiracy."]),
  anime("tokyo-revengers", "Tokyo Revengers", "東京リベンジャーズ", "b", ["Japanese anime", "Time leap", "Delinquent gangs"], ["Takemichi Hanagaki travels back to save his girlfriend from a tragic fate."]),
  anime("made-in-abyss", "Made in Abyss", "メイドインアビス", "b", ["Japanese anime", "Giant hole in earth", "Cute but dark adventure"], ["Riko and Reg descend the Abyss seeking her mother at the bottom."]),

  // ─── Anime Tier C ─────────────────────────────────────────────
  anime("my-dress-up-darling", "My Dress-Up Darling", "その着せ替え人形は恋をする", "c", ["Japanese anime", "Cosplay romance", "Shy craftsman"], ["Wakana Gojo helps Marin Kitagawa create cosplay outfits."]),
  anime("soul-eater", "Soul Eater", "ソウルイーター", "c", ["Japanese anime", "Weapon partners", "Shinigami academy"], ["Students at Death Weapon Meister Academy collect evil souls."]),
  anime("black-butler", "Black Butler", "黒執事", "c", ["Japanese anime", "Victorian demon butler", "Revenge contract"], ["Ciel Phantomhive serves the queen with help from demon butler Sebastian."]),
  anime("noragami", "Noragami", "ノラガミ", "c", ["Japanese anime", "Minor god for hire", "Five-yen wishes"], ["Yato is a stray god trying to build his own shrine."]),
  anime("durarara", "Durarara!!", "デュラララ!!", "c", ["Japanese anime", "Ikebukuro urban legend", "Headless rider"], ["Many characters' stories intertwine in Ikebukuro including Celty the dullahan."]),
  anime("bakuman", "Bakuman", "バクマン。", "c", ["Japanese anime", "Creating manga", "Two friends as artist-writer"], ["Moritaka and Akito aim to get their manga serialized in Jump."]),
  anime("parasyte", "Parasyte", "寄生獣", "c", ["Japanese anime", "Alien parasites", "Hand becomes sentient"], ["Shinichi Izumi's right hand is taken over by a parasite named Migi."]),
  anime("ergo-proxy", "Ergo Proxy", "エルゴプラクシー", "c", ["Japanese anime", "Post-apocalyptic mystery", "Philosophical sci-fi"], ["Re-l Mayer investigates awakened monsters called Proxies."]),
  anime("flcl", "FLCL", "フリクリ", "c", ["Japanese anime", "Surreal coming-of-age", "Guitar from head"], ["Naota Nandaba's mundane life is disrupted by Haruko Haruhara and robots."]),
  anime("kill-la-kill", "Kill la Kill", "キルラキル", "c", ["Japanese anime", "Living uniforms", "Over-the-top action"], ["Ryuko Matoi seeks her father's killer at Honnouji Academy."]),
  anime("promare", "Promare", "プロメア", "c", ["Japanese anime film", "Firefighters vs mutants", "Trigger studio style"], ["Lio Fotia and Galo Thymos clash as Burnish mutants face extinction."]),
  anime("weathering-with-you", "Weathering with You", "天気の子", "c", ["Japanese anime film", "Rain and sunshine girl", "Tokyo floods"], ["Hodaka meets Hina, who can clear the sky with her prayers."]),
  anime("a-silent-voice", "A Silent Voice", "聲の形", "c", ["Japanese anime film", "Bullying redemption", "Deaf classmate"], ["Shoya Ishida tries to make amends with Shoko Nishimiya."]),
  anime("perfect-blue", "Perfect Blue", "パーフェクトブルー", "c", ["Japanese anime film", "Psychological thriller", "Idol turns actress"], ["Mima Kirigoe's reality blurs as she becomes a target of obsession."]),
  anime("paprika", "Paprika", "パプリカ", "c", ["Japanese anime film", "Dream technology", "Surreal imagery"], ["Therapists use DC Mini devices until dreams leak into reality."]),
  anime("violet-evergarden", "Violet Evergarden", "ヴァイオレット・エヴァーガーデン", "c", ["Japanese anime", "War orphan writer", "Letters and emotions"], ["Violet learns the meaning of love while writing letters for others."]),
  anime("banana-fish", "Banana Fish", "バナナフィッシュ", "c", ["Japanese anime", "Crime conspiracy", "1980s New York"], ["Ash Lynx investigates a drug called Banana Fish linked to his brother."]),
  anime("given", "Given", "ギヴン", "c", ["Japanese anime", "Boys' love band", "Guitar and vocals"], ["Mafuyu Sato joins a band and processes grief through music."]),
  anime("beastars", "Beastars", "BEASTARS", "c", ["Japanese anime", "Anthropomorphic school", "Wolf and rabbit"], ["Legoshi, a gray wolf, struggles with instincts in a herbivore-carnivore society."]),
  anime("odd-taxi", "Odd Taxi", "オッドタクシー", "c", ["Japanese anime", "Taxi driver mystery", "Anthropomorphic cast"], ["Odokawa's taxi rides connect strangers in a missing-girl case."]),
  anime("mashle", "Mashle: Magic and Muscles", "マッシュル", "c", ["Japanese anime", "Magic school parody", "Muscles beat spells"], ["Mash Burnedead survives a magic world using only physical strength."]),
  anime("undead-unluck", "Undead Unluck", "アンデッドアンラック", "c", ["Japanese anime", "Supernatural negators", "Loop and immortality"], ["Andy and Fuuko seek the Union's truth while hunted for their unluck."]),
  anime("dandadan", "Dandadan", "ダンダダン", "c", ["Japanese anime", "Aliens and ghosts", "Romantic comedy action"], ["Momo and Okarun battle aliens and yokai while navigating high school."]),
  anime("hellsing", "Hellsing", "ヘルシング", "c", ["Japanese anime", "Vampire organization", "Alucard serves Integra"], ["The Hellsing Organization fights supernatural threats in England."]),
  anime("trigun", "Trigun", "トライガン", "c", ["Japanese anime", "Bounty on pacifist", "Desert planet"], ["Vash the Stampede is a gunslinger with a huge bounty and a peaceful heart."]),
  anime("samurai-champloo", "Samurai Champloo", "サムライチャンプルー", "c", ["Japanese anime", "Hip-hop samurai road trip", "Edo-era Japan"], ["Fuu hires two swordsmen to find a sunflower samurai."]),
  anime("serial-experiments-lain", "Serial Experiments Lain", "シリアルエクスペリメンツレイン", "c", ["Japanese anime", "Internet reality blur", "Early cyberpunk"], ["Lain Iwakura explores the Wired and questions identity."], ["Lain"]),
  anime("cardcaptor-sakura", "Cardcaptor Sakura", "カードキャプターさくら", "c", ["Japanese anime", "Magical cards", "Young girl captures Clow Cards"], ["Sakura Kinomoto must capture magical cards she accidentally released."]),
  anime("ranma", "Ranma ½", "らんま1/2", "c", ["Japanese anime", "Gender-bending curse", "Martial arts comedy"], ["Ranma Saotome turns into a girl when splashed with cold water."]),
  anime("inuyasha", "Inuyasha", "犬夜叉", "c", ["Japanese anime", "Feudal Japan demons", "Time-traveling schoolgirl"], ["Kagome and half-demon Inuyasha seek shards of the Shikon Jewel."]),
  anime("rurouni-kenshin", "Rurouni Kenshin", "るろうに剣心", "c", ["Japanese anime", "Meiji-era swordsman", "Reverse-blade sword"], ["Himura Kenshin wanders Japan protecting people without killing."]),
  anime("bleach-thousand-year", "Bleach: Thousand-Year Blood War", "BLEACH 千年血戦篇", "c", ["Japanese anime", "Sequel arc animated", "Quincy war returns"], ["Ichigo faces the Wandenreich's invasion in the final Bleach arc."]),
  ...
  // ─── TV Tier B ────────────────────────────────────────────────
  tv("squid-game", "Squid Game", "b", ["Korean series", "Deadly children's games", "Debt and prize money"], ["Contestants play childhood games for money with lethal penalties."], "오징어 게임"),
